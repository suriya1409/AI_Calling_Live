...
def create_dummy_call(phone_number: str, language: str, borrower_id: Optional[str] = None) -> dict:
    ...
    try:
        ...
        # Also update the borrower record with the call status
        try:
            collection = db.get_collection("borrowers")
            if collection is not None and borrower_id:
                collection.update_one(
                    ...
                    {"$set": {
                        ...
                        "ai_summary": ai_analysis.get("summary", "Call completed successfully.") if ai_analysis else "Call completed."
                    }}
                )
        except Exception as e:
            print(f"[DB] ❌ Failed to update Borrower status: {e}")
...
async def reset_calls(current_user: dict = Depends(get_current_user)):
    """Reset all call statuses in the database"""
    try:
        collection = db.get_collection("borrowers")
        if collection is not None:
            ...
            return {"status": "success", "message": "All call statuses have been reset"}
